...
export default function Promotions() {
    ...
    <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-5xl max-h-[95vh] sm:max-h-[90vh] flex flex-col overflow-hidden shadow-2xl animate-fade-in border border-orange-100">
        {/* Header */}
        <div className="px-4 sm:px-8 py-4 sm:py-6 border-b flex justify-between items-center bg-[#FFFAF0]" style={{ borderColor: '#FFD700' }}>
            <div>
                <h3 className="text-base sm:text-xl font-bold flex items-center gap-2" style={{ color: '#FF8C00' }}>
                    {editingPromotion ? <Edit className="w-4 h-4 sm:w-5 sm:h-5" /> : <Plus className="w-4 h-4 sm:w-5 sm:h-5" />}
                    <span className="text-sm sm:text-xl">{editingPromotion ? 'Modifier la stratégie' : 'Créer une nouvelle stratégie'}</span>
                </h3>
                <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] text-orange-300 mt-1">Concepteur de campagne</p>
            </div>
            <button onClick={() => setIsModalOpen(false)} className="p-1.5 sm:p-2 hover:bg-orange-100 rounded-full transition-colors text-gray-400">
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 sm:space-y-8 custom-scrollbar">
                <section className="space-y-4 sm:space-y-6">
                    <div className="grid grid-cols-1 gap-4 sm:gap-6">
                        <div className="space-y-2">
                            <label className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-400 pl-1">Nom</label>
                            <input
                                value={formData.name}
                                onChange={e => setFormData({ ...formData, name: e.target.value })}
                                className="w-full px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl border-2 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 outline-none transition-all font-semibold text-sm sm:text-base"
                                style={{ borderColor: '#FFD700' }}
                                placeholder="ex. Spécial Aïd"
                            />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-400 pl-1">Visibilité</label>
                            <div className="flex flex-col sm:flex-row gap-2">
                                <select
                                    value={formData.status}
                                    onChange={e => setFormData({ ...formData, status: e.target.value as any })}
                                    className="flex-1 px-4 py-2.5 sm:py-3 rounded-xl border-2 font-bold text-sm outline-none"
                                    style={{ borderColor: '#FFD700' }}
                                >
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2.5 sm:py-3 rounded-xl border-2 font-bold text-[9px] sm:text-[10px] uppercase transition-all ${formData.is_active ? 'bg-orange-500 border-orange-500 text-white' : 'bg-gray-50 border-gray-200 text-gray-400'}`}
                                >{formData.is_active ? 'En ligne' : 'Hors ligne'}</button>
                            </div>
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-400 pl-1">Description</label>
                        <textarea
                            value={formData.description}
                            onChange={e => setFormData({ ...formData, description: e.target.value })}
                            className="w-full px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl border-2 min-h-[60px] sm:min-h-[80px] text-xs sm:text-sm resize-none outline-none font-medium text-gray-600"
                            style={{ borderColor: '#FFD700' }}
                ...
                <section className="p-4 sm:p-6 rounded-2xl bg-orange-50 border border-orange-100 space-y-4 sm:space-y-6">
                    <div className="flex flex-col gap-4 sm:gap-6">
                        <div className="flex-1 space-y-3">
                            <label className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-orange-600">Modèle</label>
                            <div className="grid grid-cols-3 gap-2">
                                {(['percentage', 'fixed_amount', 'combo_fixed_price'] as const).map(t => (
                                    <button
                                        key={t}
                                        onClick={() => setFormData({ ...formData, promotion_type: t })}
                                        className={`p-2 sm:p-3 rounded-xl border-2 flex flex-col items-center gap-1 sm:gap-2 transition-all ${formData.promotion_type === t ? 'border-orange-500 bg-white text-orange-600 shadow-sm' : 'border-orange-100 bg-white/50 text-gray-400 hover:border-orange-200'}`}
                                    >
                                        {t === 'percentage' && <Percent size={16} className="sm:w-[18px] sm:h-[18px]" />}
                                        {t === 'fixed_amount' && <DollarSign size={16} className="sm:w-[18px] sm:h-[18px]" />}
                                        {t === 'combo_fixed_price' && <Package size={16} className="sm:w-[18px] sm:h-[18px]" />}
                                        <span className="text-[8px] sm:text-[9px] font-black uppercase whitespace-nowrap">{t.split('_')[0]}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                        <div className="flex-1 space-y-3">
                            <label className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-orange-600">Valeur</label>
                            <div className="relative">
                                <input
                                    type="number"
                                    value={formData.value}
                                    onChange={e => setFormData({ ...formData, value: e.target.value })}
                                    className="w-full px-4 sm:px-5 py-3 sm:py-4 rounded-xl border-2 text-xl sm:text-2xl font-black outline-none"
                                    style={{ borderColor: '#FFD700' }}
                                    placeholder="0.00"
                                />
                                <span className="absolute right-4 sm:right-5 top-1/2 -translate-y-1/2 font-black text-orange-400 text-base sm:text-lg">{formData.promotion_type === 'percentage' ? '%' : 'DA'}</span>
                            </div>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 pt-3 sm:pt-4 border-t border-orange-200/30">
                        <div className="space-y-2">
                            <label className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-orange-400 flex items-center gap-1">
                                <Calendar size={10} /> Date de début
                            </label>
                            <input 
                                type="datetime-local" 
                                value={formData.start_date} 
                                onChange={e => setFormData({ ...formData, start_date: e.target.value })} 
                                className="w-full p-2 sm:p-2.5 rounded-lg border border-orange-200 bg-white text-[11px] sm:text-xs font-bold" 
                            />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-orange-400 flex items-center gap-1">
                                <Clock size={10} /> Date de fin
                            </label>
                            <input 
                                type="datetime-local" 
                ...
                {/* Inventory Selector - Mobile Version */}
                <div className="bg-gray-50 rounded-2xl border border-gray-100 flex flex-col p-4 sm:p-6 space-y-4 sm:space-y-6">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                        <h4 className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-400">Catalogue</h4>
                        <div className="relative w-full sm:w-auto">
                            <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-300" />
                            <input
                                value={itemSearchQuery}
                                onChange={e => setItemSearchQuery(e.target.value)}
                                className="pl-8 pr-3 py-1.5 rounded-lg border bg-white text-[10px] outline-none w-full sm:w-32 sm:focus:w-40 transition-all font-bold"
                                placeholder="RECHERCHE RAPIDE"
                            />
                        </div>
                    </div>

                    <div className="max-h-60 sm:max-h-80 overflow-y-auto space-y-3 custom-scrollbar pr-2">
                        {formData.promotion_type === 'combo_fixed_price' ? (
                            <div className="space-y-4 sm:space-y-6">
                                {/* Combo Status */}
                                <div className="p-3 sm:p-4 rounded-xl bg-gray-900 text-white space-y-3 shadow-lg">
                                    <div className="flex items-center gap-2 text-orange-500">
                                        <Package size={12} className="sm:w-[14px] sm:h-[14px]" />
                                        <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.15em] sm:tracking-[0.2em]">Pack en cours</span>
                                    </div>
                                        ...
                                        {formData.combo_items.map((ci, idx) => {
                                            ...
                                            return (
                                                <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-white/5 border border-white/5">
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] sm:text-[11px] font-bold line-clamp-1">{itm?.name}</span>
                                                        <span className="text-[7px] sm:text-[8px] opacity-40 uppercase">{sz?.size || 'Standard'} (x{ci.quantity})</span>
                                                    </div>
                                                    <button onClick={() => {
                                ...
                                <div className="space-y-2">
                                    {filteredMenuItems.map(item => (
                                        <div key={item.id} className="p-2.5 sm:p-3 bg-white rounded-xl border border-gray-100 hover:border-orange-300 transition-all cursor-default">
                                            <h5 className="text-[10px] sm:text-[11px] font-black">{item.name}</h5>
                                            {item.sizes && item.sizes.length > 0 ? (
                                                <div className="flex flex-wrap gap-1 mt-2">
                                                    {item.sizes.map(sz => (
                                                        <button 
                                                            key={sz.id} 
                                                            onClick={() => setFormData({ ...formData, combo_items: [...formData.combo_items, { menu_item: item.id, menu_item_size: sz.id, quantity: 1 }] })} 
                                                            className="px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md bg-orange-50 text-orange-600 text-[7px] sm:text-[8px] font-black hover:bg-orange-600 hover:text-white transition-all"
                                                        >
                                                            + {sz.size}
                                                        </button>
                                                    ))}
                                                ...
                                                <button 
                                                    onClick={() => setFormData({ ...formData, combo_items: [...formData.combo_items, { menu_item: item.id, quantity: 1 }] })} 
                                                    className="mt-2 w-full py-1.5 rounded-lg bg-orange-50 text-orange-600 text-[7px] sm:text-[8px] font-black hover:bg-orange-600 hover:text-white transition-all"
                                                >
                                                    AJOUTER SEUL
                                                </button>
                                            )}
                            ...
                            <div className="space-y-2 sm:space-y-3">
                                {filteredMenuItems.map(item => {
                                    ...
                                    return (
                                        <div key={item.id} className={`p-3 sm:p-4 rounded-xl border-2 transition-all ${isFullSelected || szSel > 0 ? 'bg-white border-orange-500 shadow-sm' : 'bg-white border-transparent hover:border-orange-100'}`}>
                                            <div className="flex justify-between items-start mb-2">
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] sm:text-[11px] font-bold text-gray-900">{item.name}</span>
                                                    <span className="text-[7px] sm:text-[8px] font-black text-gray-300 uppercase tracking-widest">{item.category}</span>
                                                </div>
                                                {!item.sizes?.length && (
                                                    <button
                                                        onClick={() => {
                                                        ...
                                                        className={`p-1 sm:p-1.5 rounded-lg border-2 ${isFullSelected ? 'bg-orange-500 border-orange-500 text-white' : 'border-gray-100 text-gray-200'}`}
                                                    >
                                                    ...
                                                    {item.sizes.map(sz => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                className={`py-1 sm:py-1.5 rounded flex items-center justify-between px-1.5 sm:px-2 text-[7px] sm:text-[8px] font-black uppercase tracking-tighter ${sel ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-400'}`}
                                                            >
                                                                {sz.size} {sel && <CheckCircle size={8} />}
                                                            </button>
                                                        )
                                                    })}
                                                </div>
        ...
        {/* Footer */}
        <div className="p-4 sm:p-8 border-t bg-gray-50 flex flex-col gap-4 sm:gap-6">
            {livePreview && (
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-6 p-3 sm:p-4 rounded-2xl bg-white border border-orange-200 shadow-sm">
                    <div className="flex flex-col w-full sm:w-auto">
                        <span className="text-[9px] sm:text-[10px] font-black text-orange-300 uppercase">Aperçu de l'impact</span>
                        <div className="flex items-baseline gap-2">
                            <span className="text-xl sm:text-2xl font-black text-gray-900">{formatCurrency(livePreview.final)}</span>
                            <span className="text-xs text-gray-400 line-through">{formatCurrency(livePreview.original)}</span>
                        </div>
                    </div>
                    <div className="hidden sm:block h-10 w-px bg-orange-100" />
                    <div className="flex flex-col w-full sm:w-auto">
                        <span className="text-[9px] sm:text-[10px] font-black text-green-500 uppercase">Efficacité</span>
                        <span className="text-xs sm:text-sm font-bold text-green-600">Économie de {formatCurrency(livePreview.saved)} (-{((livePreview.saved / livePreview.original) * 100).toFixed(0)}%)</span>
                    </div>
                </div>
            )}
            <div className="flex flex-col sm:flex-row gap-3 w-full">
                <button 
                    ...
                    className="w-full sm:w-auto sm:flex-none px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-bold uppercase tracking-widest text-sm sm:text-base text-gray-400 hover:text-gray-600 transition-colors"
                >
                    Annuler
                </button>
                <button 
                    onClick={handleSave} 
                    className="w-full sm:flex-1 px-6 sm:px-12 py-3 sm:py-4 rounded-xl text-white font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] text-xs sm:text-sm shadow-xl hover:shadow-orange-200 transition-all active:scale-95 group flex items-center justify-center gap-2 sm:gap-3" 
                    style={{ backgroundColor: '#FF8C00' }}
                >
                    {submitting ? (
                        ...
                        <>
                            <span className="text-xs sm:text-sm">{editingPromotion ? 'Mettre à jour' : 'Lancer la campagne'}</span>
