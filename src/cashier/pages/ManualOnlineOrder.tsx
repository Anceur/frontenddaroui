...
import { Plus, Minus, ShoppingCart, Loader2, CheckCircle, User, Phone, MapPin, Truck, ShoppingBag, X } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">Créer des commandes de livraison ou à emporter</p>
      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {boxes.map((promo) => (
                    <div key={promo.id} className="border-2 border-blue-200 bg-blue-50 rounded-lg p-4 relative overflow-hidden">
                       <div className="absolute top-0 right-0 bg-blue-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-bl">MENU</div>
                  ...
                  {cart.map((item, index) => (
                    <div key={index} className="flex justify-between items-start border-b pb-2">
                      ...
                      <div className="text-right">
                         ...
                         <button onClick={() => removeFromCart(index)} className="text-red-500 hover:text-red-700 text-xs mt-1">Retirer</button>
                      </div>
                    </div>
                  ))}
                </div>
