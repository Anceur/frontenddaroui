...
function ReceiptModal({ order, isOpen, onClose }: { order: any; isOpen: boolean; onClose: () => void }) {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            ...
            const customerName = order.loyalCustomer?.name || order.customer || 'Client non enregistré';
        ...
        <p><strong>Carte de fidélité :</strong> ${order.loyalCustomer.loyaltyCardNumber || order.loyalCustomer.loyalty_card_number || 'Non applicable'}</p>
      ` : '';
        ...
        <html>
          ...
          <body>
            <div class="header"><h1 class="title">RESTAURANT</h1><p class="subtitle">Reçu / Ticket de caisse</p></div>
            <div class="info">
            ...
            <div class="totals">
                ...
                ${order.tax_amount ? `<div class="row"><span>TVA</span><span>${Number(order.tax_amount).toFixed(2)} DA</span></div>` : ''}
            ...
            {loading ? (
                ...
                <div className="space-y-8">
                    ...
                    {history?.offline_orders && history.offline_orders.length > 0 && (
                        <div>
                            <h3 className="text-xl font-semibold mb-4 text-amber-600 flex items-center gap-2"><ShoppingCart size={20} /> Commandes hors ligne ({history.offline_orders.length})</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
