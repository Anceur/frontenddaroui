...
export default function OrderTypeDrawer({
  ...
  const handleDineInOrder = async () => {
    ...
    if (!tableNumber) {
      setError("Table number is missing. Please refresh the page.")
      return
    }

    // Prepare offline order data (no customer data needed)
    const orderData = {
    ...
    // Validate order data
    const validation = OrderService.validateOfflineOrderData(orderData)
    ...
    try {
      const response = await OrderService.submitOfflineOrder(orderData)
      ...
      console.error("Error placing offline order:", err)
